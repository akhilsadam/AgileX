...
const ext = ".txt";
const htm = "currentTex.html"

const classname = "CodeMirror"; //"module-input-code";
const dataname = "module-input-code";
const outname = "embeds";
const checkout = "checkout";
const sep2 = " : ";

function onclick_function(item, id) {
    // console.log("OnClick_Function",item);
    // console.log("branch", item.branches);
    let path;
    let cname;
    if ('HEAD' == item.refs[0]) {
        path = reg + id + "/";
        ...
        path = WT + id + "-" + item.hashAbbrev + "/";
        console.log("Commit:", id, "hashabbrev", item.hashAbbrev);
        cname = item.hashAbbrev + sep2 + id;
    }
    console.log(path);

    var elements = document.getElementsByClassName(classname);
    // console.log(elements[id].CodeMirror)//.getDoc().setValue(open(path+id+ext));

    var url = document.URL + '..' + path + id + ext;
    var url2 = '/' + '..' + path + htm;
    // console.log("url",url);
    ...
    fetch(url)
        .then(function(response) {
            response.text().then(function(text) {
                // console.log(text);
                elements[id].CodeMirror.doc.setValue(text);
                document.getElementsByClassName(dataname)[id].innerText = text;
            });
        }).stopImmediatePropagation; // stopImmediate is so that we do not open a new tab

    document.getElementsByClassName(outname)[id].setAttribute('src', url2);
    document.getElementsByClassName(checkout)[id].innerText = cname;

    // elements[id].value = open(path+id+ext);
    elements[id].setAttribute("data-path", path);
    elements[id].setAttribute("data-id", id);
    // console.log("textfile",path+id+ext)
    // console.log("textfile",open(path+id+ext))
    // console.log(elements[id]);
    // console.log(elements[id].CodeMirror.value);
...
// changed below for div_id and onclick_function..

// Regular gitgraph libraries follow (with the exception of changes for the above callback).
    ...
    var commit = createCommonjsModule(function(module, exports) {
        ...
        class Commit {
            constructor(options) {
                    ...
                    // Set callbacks
                    this.onClick = () => (options.onClick ? options.onClick(this) : onclick_function(this, options.style.div_id));
        ...
        class GitgraphCore {
            ...
            createBranch(args) {
                    ...
                    if (typeof args === "string") {
                        ...
                        options = Object.assign({}, options, args, { parentCommitHash: this.refs.getCommit(parentBranchName), style: Object.assign({}, options.style, args.style, { label: Object.assign({}, options.style.label, args.style.label) }) });
                    }
